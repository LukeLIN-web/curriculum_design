...
现代处理器使用的是一种称为 **虚拟寻址(Virtual Addressing)** 的寻址方式。完成虚拟地址转换为物理地址转换的硬件是 CPU 中含有一个被称为 **内存管理单元（Memory Management Unit, MMU）** 的硬件。利用存放在 主存中的查询表来动态翻译虚拟地址, 这个表的内容是操作系统管理的. 叫做页表
